...
compressResult * bbcCompress(unsigned char * to_compress, int size){
  ...
  blockSeg * param = (blockSeg *) malloc(sizeof(blockSeg));

  param->size = size;
  param->toCompress = to_compress;

  compressResult * compress = (compressResult *) malloc(sizeof(compressResult));
  compress->compressed_seq = (unsigned char *) malloc(sizeof(unsigned char));
  compress->size = 0;
